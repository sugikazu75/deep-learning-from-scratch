...
class Multi_Layer_NeuralNetwork:
    def __init__(self, input_size, hidden_size1, output_size):
        # init layers
        # Affine, ReLU , Affine
        affine_init_param = [[np.random.randn(input_size, hidden_size1), np.zeros(hidden_size1)], [np.random.randn(hidden_size1, output_size), np.zeros(output_size)]]
        self.layers = [Affine(affine_init_param[0][0], affine_init_param[0][1]), ReLU(), Affine(affine_init_param[1][0], affine_init_param[1][1])]
        # lastlayer
